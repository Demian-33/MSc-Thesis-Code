...
# las obs. de las alcaldias 8 y 15 son para prueba, el resto son entrenamiento
...
# resumen posterior personalizado
...
# (1) Modelo log-normal ====
file <- file.path('LogSN-rho01-SSVS-oneInt.stan') # pronostico, un int
file <- file.path('LogSN-rho01-SSVS.stan') # pronostico
...
init_list <- list(
  ...
  y_mis = predict(r0, newdata = dat$X_mis)
)

## (1.3) Correr el modelo con VB ====
t0 <- proc.time()
muestra_VB <- mod_sn$variational(
  output_dir = r'(C:\Users\Lesau\Documents\EntornosR\log-normal\VB-pronostico)',
  data = dat,
  seed = 123,
  init = list(init_list),
  draws = 1000,
...
muestra_vb <- muestra_VB$summary(variables = out_var, out_stan)
tproc_vb <- c(tproc_vb, (proc.time()-t0)[3])
save(file=r'(C:\Maestría_CP\Tesis\Documento-Tesis-SAOM\EntornosR\muestraVB-log-normal-sesgado-pronostico1.RData)',
     muestra_VB)

## (borrar esto despues) ssvs
# ssvs <- head(posterior_prob(muestra_VB), n=100)
# cumsum(ssvs[, 1])
# x11(); plot(ssvs[, 1], type='b', lwd=2, col=4); grid();
# modelo1 <- str_split(ssvs[1, 2], "", simplify = F) %>% unlist() %>% as.numeric()
# modelo1_bool <- as.logical(modelo1)
# sum(modelo1_bool)
# colnames(Xtmp)[modelo1_bool]
##
# 
...
# 
# set.seed(1)
# datos <- get_obs(censo=T, y=tmp$ym_bin)
...
# t0 <- proc.time()
# muestra_probitVB <- mod_sn$variational(
#   output_dir = r'(C:\Users\Lesau\Documents\EntornosR\probit\VB-estimacion)',
#   data = dat,
#   seed = 123,
#   init = list(init_list),
#   draws = 1000,
#   adapt_iter=2000,
#   adapt_engaged=F, # TRUE
#   grad_samples = 1,
#   elbo_samples = 150,
#   eta=0.1,
#   iter=75000, # 75000
...
#   algorith='meanfield', # fullrank
# )
# muestra_vb <- muestra_probitVB$summary(variables = out_var, out_stan)
# # muestra_vb <- muestra_probitVB$summary(variables = c('rho', 'b'), out_stan) # sin mu
# tproc_vb <- c(tproc_vb, (proc.time()-t0)[3])
# save(file=r'(C:\Maestría_CP\Tesis\Documento-Tesis-SAOM\EntornosR\muestraVB-probit-sesgado-estimacion.RData)',
#      muestra_probitVB)
# 
# # print(muestra_vb, n=18)
# # metrics_cat(findInterval(apply(muestra_probitVB$draws('z_all'), 2, mean)[datos$master], 0), datos$y_obs)
# # table(findInterval(apply(muestra_probitVB$draws('z_all'), 2, mean)[datos$master], 0), datos$y_obs)
# 
...
# set.seed(1)
# datos <- get_obs(censo=T, y=tmp$ym_ord)
# dat <- list(
#   K = K,
#   N_obs = length(datos$y_obs),
#   N_mis = length(datos$y_mis),
...
#   w_tilde=rnorm(K, 0.5, sd=0.1)
# )
# ## (3.3) Correr el modelo con VB ====
# t0 <- proc.time()
# muestra_probitordVB <- mod_sn$variational(
#   output_dir = r'(C:\Users\Lesau\Documents\EntornosR\probit-ordenado\VB-estimacion)',
#   data = dat,
#   seed = 123,
#   init = list(init_list),
#   draws = 1000,
...
#   algorith='meanfield', # fullrank
# )
# muestra_vb <- muestra_probitordVB$summary(variables = out_var, out_stan)
# tproc_vb <- c(tproc_vb, (proc.time()-t0)[3])
# save(file=r'(C:\Maestría_CP\Tesis\Documento-Tesis-SAOM\EntornosR\muestraVB-probit-ordenado-sesgado-estimacion.RData)',
#      muestra_probitordVB)
# 
writeLines(paste0(tproc_vb, collapse = '~'),
           con=r'(C:\Maestría_CP\Tesis\Documento-Tesis-SAOM\EntornosR\muestraVB-tiempos-pronostico1.txt)')

cat('Done!\n')

# (4) cargar datos ====
# rm(list=ls())
# load(r'(C:\Maestría_CP\Tesis\Documento-Tesis-SAOM\EntornosR\muestraHMC-log-normal-sesgado-pronostico1.RData)')
# out_var <- c('rho','sigma', 'mu')
# muestra_hmc <- fit_hmc$summary(variables = out_var, out_stan)
