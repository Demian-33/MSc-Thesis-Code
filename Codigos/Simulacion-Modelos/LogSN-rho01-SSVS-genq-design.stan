...
model {
  ...
  target +=normal_id_glm_lupdf(y_obs | X_obs,  intercept, b, sig);
  ...
  // target +=normal_id_glm_lupdf(y_mis | X_mis,  interceptmis, b, sigmis);
}
...
generated quantities {
  ...
  for(i in 1:(N_obs+N_mis)){
    j = group_all[i];
    //eta_ij = X_mis[i]*b + rho[j] .* z[j]- sigma .* rho[j] .* sqrt(2/pi()) ./ sqrt(1 - 2/pi() * square(rho[j]));
    eta_ij = X_all[i]*b + rho[j] .* z[j]- sigma .* rho[j] .* sqrt(2/pi()) ./ sqrt(1 - 2/pi() * square(rho[j]));
