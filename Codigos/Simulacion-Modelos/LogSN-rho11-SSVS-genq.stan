...
parameters {
  ...
  vector<lower=-1, upper=1>[K] rho;      // Correlation coefficient for each group
...
vector<lower=0>[K] z = z_std .* sigma ./ sqrt(1 - 2*square(rho)/pi());
}
...
model {
  ...
  // prior propia y no informativa para sigma
  ...
  vector[N_obs] intercept = rho[group].* z[group] - sigma*rho[group].* sqrt(2/pi()) ./ sqrt(1 - 2*square(rho[group])/pi());
  ...
  // target +=normal_id_glm_lupdf(y_mis | X_mis, mu[groupmis] + interceptmis, b, sigmis);
}
...
generated quantities {
  ...
  for(i in 1:(N_obs+N_mis)){
    j = group_all[i];
    //eta_ij = mu[j] + X_mis[i]*b + rho[j] .* z[j]- sigma .* rho[j] .* sqrt(2/pi()) ./ sqrt(1 - 2/pi() * square(rho[j]));
