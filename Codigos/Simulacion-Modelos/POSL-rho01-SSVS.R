...
# Parametrización:
#   Centrada:
#   Z ~ SN(0, 1, \rho/\sqrt{1-\rho^2})
#   Y ~ SN(\mu, \sigma^2, \rho/\sqrt{1-\rho^2})
#   E(Z)   = \sigma\rho\sqrt(2/\pi)
#   Var(Z) = \sigma^2 (1 - 2\rho^2/\pi)^2
#   Y = mu + sigma (Y-E(Z))/\sqrt{Var(Z)}
# Vale la pena notar que:
#   y_{ij} depende del valor de z_{ij} (var. latente), además
#   z_{ij} se genera a partir de truncamiento oculto, es decir que
#   z_{ij} se genera a través de [U_{i}, W_{i}]^T, cuando w_{i} > 0,
...
model {
  // Here we consider center parameterization
  ...
  // Wide prior for the threshold, restrictions are already in it
  delta1 ~ normal(0, 100);
