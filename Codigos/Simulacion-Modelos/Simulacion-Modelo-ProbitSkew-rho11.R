...
for(int in interceptos){
  ...
  if(int == 'unico'){
    mod_sn <- cmdstan_model(stan_file='ProbitSN-rho11-SSVS-genq-oneInt.stan', compile=T) # puede tener futuro
  } else{
  if(int == 'no'){
...
for(per in porcentaje){
  ...
  ## (4.1) Fijar rutas ====
  path  <- paste0('./probit-sesgado-sim2/muestra_VB_', per, '_', int,  '.RData')
  path0 <- paste0('./probit-sesgado-sim2/muestra_vb_', per, '_', int, '.csv')
  path1 <- paste0('./probit-sesgado-sim2/predict_vb_', per, '_', int, '.csv')
  path2 <- paste0('./probit-sesgado-sim2/SSVS_vb_', per, '_', int, '.csv')
  path3 <- paste0('./probit-sesgado-sim2/mar_freq_vb_', per, '_', int, '.csv')
  ...
  ## (5.1) Fijar rutas ====
  path  <- paste0('./probit-sesgado-sim2/muestra_HMC_', per, '_', int,  '.RData')
  path0 <- paste0('./probit-sesgado-sim2/muestra_hmc_', per, '_', int,  '.csv')
  path1 <- paste0('./probit-sesgado-sim2/predict_hmc_', per, '_', int,  '.csv')
  path2 <- paste0('./probit-sesgado-sim2/SSVS_hmc_', per, '_', int,  '.csv')
  path3 <- paste0('./probit-sesgado-sim2/mar_freq_hmc_', per, '_', int,  '.csv')
